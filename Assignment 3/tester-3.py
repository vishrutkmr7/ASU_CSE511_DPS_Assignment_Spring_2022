...
INPUT_FILE_PATH = (
    "/Users/vishrutjha/Documents/ASU/CSE 511 DPS/Assignment 3/text_data.txt"
)
...
if __name__ == "__main__":
    try:
        ...
        with testHelper.getOpenConnection(dbname=DATABASE_NAME) as conn:
            ...
            [result, e] = testHelper.testrangepartition(
                MyAssignment, RATINGS_TABLE, 5, conn, 0, ACTUAL_ROWS_IN_INPUT_FILE
            )
            if result:
                print("rangepartition function pass!")

            # ALERT:: Use only one at a time i.e. uncomment only one line at a time and run the script
            # [result, e] = testHelper.testrangeinsert(
            #     MyAssignment, RATINGS_TABLE, 100, 2, 3, conn, "2"
            # )
            # [result, e] = testHelper.testrangeinsert(
            #     MyAssignment, RATINGS_TABLE, 100, 2, 0, conn, "0"
            # )
            if result:
            ...
            [result, e] = testHelper.testroundrobinpartition(
                MyAssignment, RATINGS_TABLE, 5, conn, 0, ACTUAL_ROWS_IN_INPUT_FILE
            )
            if result:
            ...
            # [result, e] = testHelper.testroundrobininsert(
            #     MyAssignment, RATINGS_TABLE, 100, 1, 3, conn, "1"
            # )
            # [result, e] = testHelper.testroundrobininsert(
            #     MyAssignment, RATINGS_TABLE, 100, 1, 3, conn, "2"
            # )
            if result:
