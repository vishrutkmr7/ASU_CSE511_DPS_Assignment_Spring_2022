...
def loadRatings(ratingstablename, ratingsfilepath, openconnection):
    ...
    cur.execute(
        "CREATE TABLE "
        + ratingstablename
        + " (UserID INT, temp1 VARCHAR(10),  MovieID INT , temp3 VARCHAR(10),  Rating REAL, temp5 VARCHAR(10), Timestamp INT)"
    )
    ...
    cur.execute(
        "ALTER TABLE "
        + ratingstablename
        + " DROP COLUMN temp1, DROP COLUMN temp3,DROP COLUMN temp5, DROP COLUMN Timestamp"
    )
...
def rangePartition(ratingstablename, numberofpartitions, openconnection):
    ...
    try:
        ...
        while MinRating < MaxRating:
            ...
            if lowerLimit == 0.0:
                cursor.execute(
                    "SELECT * FROM %s WHERE Rating >= %f AND Rating <= %f"
                    % (ratingstablename, lowerLimit, upperLimit)
                )
...
def roundRobinPartition(ratingstablename, numberofpartitions, openconnection):
    ...
    try:
        ...
        if not bool(cursor.rowcount):
            ...
            return
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS RoundRobinRatingsMetadata(PartitionNum INT, TableNextInsert INT)"
        )
        x = 0
        ...
        for row in rows:
            if x < upperLimit:
                ...
                y = lastInserted
        cursor.execute(
            "INSERT INTO RoundRobinRatingsMetadata (PartitionNum, TableNextInsert) VALUES(%d,%d)"
